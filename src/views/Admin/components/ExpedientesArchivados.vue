        ...
        <base-header type="gradient-info" class="pb-6 pb-8 pt-5 pt-md-8">
            ...
            <b-row >
                <b-col class="justify-content-center pb-5">
                    <p class="welcome">EXPEDIENTES ARCHIVADOS - MDA</p>
                </b-col>
            </b-row>
                            ...
                            <tbody>
                                <tr v-for="(archivation, index) in archivations" :key="`${index}-ext-exp-ent`">
                                    <th scope="row">{{ archivation.attributes.expedient_code }}</th>
                                     ...
                                     <td>  {{ archivation.attributes.user_name  }}<br>{{ archivation.attributes.user_area  }} </td>
...
<script>
import { getExpedientsArchivationsAll } from '@/api/expedient'
...
export default {
    ...
    methods: {
        cargarDatos (pPage) {
           ...
           getExpedientsArchivationsAll(this.$store.state.user.data.id, pPage)
                .then ((response) => {
